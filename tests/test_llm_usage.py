...
from vela.config import LlmConfig, VelaConfig, load_config
from vela.llm import calculate_cost, create_llm_client, get_builtin_price_profile
...
def test_agent_propagates_connection_error_without_raising(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("vela.agent.agent.SnapshotService", NoopSnapshotService)
    monkeypatch.setattr(httpx.AsyncClient, "stream", fail_stream)
    config = VelaConfig()
...
def test_unknown_model_context_and_prices_can_be_configured(tmp_path) -> None:
    project_config = tmp_path / ".vela" / "config.json"
