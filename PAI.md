...
本文件是 Vela Python 的项目级长期上下文，供 Agent、新线程和协作者首读。只记录跨任务稳定的
...
- 项目名：Vela Python
- 定位：运行在终端中的 Python AI Agent CLI，对标 Claude Code，并与 Vela Java/TypeScript
  的公开能力保持一致。
...
uv run vela --mode plan -p "先规划再执行"
uv run vela --mode team --worker-mode plan -p "并行审计核心模块"
...
- CLI/TUI 行为不能只做单元测试：条件允许时还要运行真实 `vela` 命令或终端输入流程。
