...
class AgentOrchestrator:
    ...
    def __init__(
        ...
        *,
        ...
        plan_review_callback: (
            Callable[[list[ExecutionStep]], PlanReviewDecision | Awaitable[PlanReviewDecision]]
            | None
        ) = None,
    ):
        self.llm_client = llm_client
    ...
    async def run(self, message: str) -> AsyncIterator[dict[str, Any]]:
        ...
        self.total_usage = Usage()
        self.total_turns = 0
        self.history = [Message(role="user", content=message)]
        try:
            planning_task = message
            while True:
                ...
                if self.plan_review_callback is not None:
                    yield {"type": "plan_review"}
                decision = await self._review_plan(steps)
    ...
    async def _review_plan(self, steps: list[ExecutionStep]) -> PlanReviewDecision:
        return await resolve_plan_review(self.plan_review_callback, steps)
    ...
    async def _run_step_with_worker_queue(
        ...
        try:
            ...
            self.history.extend(bounded_tool_transcript(worker.history))
